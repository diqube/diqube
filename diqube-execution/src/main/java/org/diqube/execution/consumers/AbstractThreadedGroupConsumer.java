...
/**
 * Abstract base class of {@link GroupConsumer}s that handles calling the {@link ExecutablePlanStep} correctly when
 * there is new data.
 * 
...
public abstract class AbstractThreadedGroupConsumer extends AbstractPlanStepBasedGenericConsumer implements
  ...
  public void consumeGroups(Map<Long, List<Long>> fullGroups) {
    doConsumeGroups(fullGroups);
    if (planStep != null)
      planStep.continueProcessing();
  }

  protected abstract void doConsumeGroups(Map<Long, List<Long>> fullGroups);
}
