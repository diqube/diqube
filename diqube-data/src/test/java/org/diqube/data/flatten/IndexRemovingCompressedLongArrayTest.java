...
/**
 * Tests {@link IndexRemovingCompressedLongArray}.
 *
 * @author Bastian Gloeckle
 */
public class IndexRemovingCompressedLongArrayTest {
  ...
  public void removeNone() {
    // GIVEN
    IndexRemovingCompressedLongArray array =
        new IndexRemovingCompressedLongArray(delegate, new BitEfficientLongArray(new long[] {}, true), 0L);
  ...
  public void removeAll() {
    // GIVEN
    IndexRemovingCompressedLongArray array = new IndexRemovingCompressedLongArray(delegate,
  ...
  public void removeEven() {
    // GIVEN
    long[] indicesRemoved = LongStream.range(0, delegateUncompressed.length).filter(l -> l % 2 == 0).toArray();
    int[] indicesAvail = IntStream.range(0, delegateUncompressed.length).filter(l -> l % 2 == 1).toArray();
    IndexRemovingCompressedLongArray array =
        new IndexRemovingCompressedLongArray(delegate, new BitEfficientLongArray(indicesRemoved, true), 0L);
    ...
    for (int i = 0; i < indicesAvail.length; i++)
      decompressedAvail[i] = delegateUncompressed[indicesAvail[i]];
    ...
    Assert.assertEquals(
        array.getMultiple(Arrays.asList(0, indicesAvail.length - 1)), Arrays
            .asList(delegateUncompressed[indicesAvail[0]], delegateUncompressed[indicesAvail[indicesAvail.length - 1]]),
        "Expected correct result on getMultiple.");
  }
  ...
  public void removeOdd() {
    // GIVEN
    long[] indicesRemoved = LongStream.range(0, delegateUncompressed.length).filter(l -> l % 2 == 1).toArray();
    int[] indicesAvail = IntStream.range(0, delegateUncompressed.length).filter(l -> l % 2 == 0).toArray();
    IndexRemovingCompressedLongArray array =
        new IndexRemovingCompressedLongArray(delegate, new BitEfficientLongArray(indicesRemoved, true), 0L);
