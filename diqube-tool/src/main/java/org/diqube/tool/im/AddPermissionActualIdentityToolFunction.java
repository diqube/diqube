 ...
 */
@IsActualIdentityToolFunction(identityFunctionName = AddPermissionActualIdentityToolFunction.FUNCTION_NAME,
    shortDescription = AddPermissionActualIdentityToolFunction.DESCRIPTION)
public class AddPermissionActualIdentityToolFunction extends AbstractActualIdentityToolFunction {
  private static final Logger logger = LoggerFactory.getLogger(AddPermissionActualIdentityToolFunction.class);

  public static final String FUNCTION_NAME = "addpermission";
  public static final String DESCRIPTION = "Add a permission to a specific user." + "\n\nNeeds parameters:\n" //
  ...
  public void doExecute(Ticket ticket, Iface identityService)
    ...
    if (paramPermissionObject == null)
      logger.info("Adding permission '{}' to user '{}'", paramPermission, paramUser);
    else
      logger.info("Adding permission '{}' on '{}' to user '{}'", paramPermission, paramPermissionObject, paramUser);

    OptionalString object = new OptionalString();
    if (paramPermissionObject != null)
      object.setValue(paramPermissionObject);

    identityService.addPermission(ticket, paramUser, paramPermission, object);
  }
