...
public class ExecutionRequestOptimizer {
  ...
  public ExecutionRequest optimize() {
    if (request.getWhere() != null) {
      new OptimizerComparisonInfoEnhancer(request.getWhere()).enhanceFull();
  ...
  private ExecutionRequest optimizeWhereNot() {
    request.setWhere(whereNotOptimizer.optimize(request.getWhere()));
    return request;
  }
}
